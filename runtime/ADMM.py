...
if __name__ == '__main__':
    ...
    for i in range(nodes):
        ...
        args_node = (MAX_ITER, up_set[i], dw_set[i], nodes, A_list_dlmp, C.r, pf, dem_node[i],
                     ...
                     C.g_P_min[i], FT_f, GT_f, maxes, i, MODE, remote)
    ...
    ******************************
    ...
    for iteration in range(MAX_ITER + 1):
        ...
        total_residual = numpy.zeros(num_time)
        ...
        for n in range(nodes):
            ...
            # Then, discard local values to save us from using a nodes*nodes*num_time array
            ...
            if n:
                # this nodes local value of its incoming line flow
                local_resP = fP_n[0][1]
            else:
                # by abstraction, the feeders incoming is what flows into its children
                local_resP = -1.0*sum(fP_n[l_f][1] for l_f in range(1, len(dw_set[n])))

            net_injection = gP_curr[n] + der_array[n, :] - dP_array[n, :]
            total_residual += net_injection  # sum of all net injections should be 0
            da_price[n] += rho_pen * local_resP
            ...
            if MODE in [1, 2, 3]:
                ...
                global_A += alpha_n
                local_resA = alpha_n[n]  # just this nodes local value of its own part factors
                flex_price -= local_resA
            ...
            if MODE == 4:
                alpha_n = use_pipe.recv()
                global_A += alpha_n
                local_resA = alpha_n[n]
                flex_price -= rho_pen * local_resA

        if MODE in [1, 2, 3]:
            global_A *= n_inv
            # Do it here, since we need to do it only once
            global_resA = numpy.sum(global_A, axis=0)
            flex_price += global_resA
        ...
        if MODE == 4:
            global_A *= n_inv
            # Do it here, since we need to do it only once
            global_resA = numpy.sum(global_A, axis=1)
            flex_price += global_resA
            norm_FL[0, iteration] = numpy.linalg.norm(flex_price)
        ...
        for n in range(nodes):
            if n:
                # the global copy of this nodes incoming line flow
                global_resP = global_P[n, :]
            else:
                # same above, for the feeder we use what flows into node 1
                global_resP = -1.0*sum(global_P[l_f, :] for l_f in dw_set[n] if l_f != n)

            da_price[n] -= rho_pen * global_resP
        ...
        s_dec = 0
        sqrt_N = sqrt(nodes)
        sqrt_T = sqrt(num_time)

        for iii in range(nodes):
            r_dec += 1 if primal_rezzies[iii] * sqrt_N <= epsilon_primal else 0
            s_dec += 1 if dual_rezzies[iii] * sqrt_N <= epsilon_dual else 0

        res_dec = 0
        for ttt in range(num_time):
            # Check both ways!
            dP_ep = total_demand[ttt] * epsilon_flow
            res_dec += 1 if dP_ep >= total_residual[ttt] * sqrt_T >= -1.0 * dP_ep else 0

        rel_surplus = sum(total_residual) * 100 / sum(total_demand)
        surplus_dec = 1 if epsilon_surplus >= rel_surplus >= -1.0 * epsilon_surplus else 0
        ...
        if r_dec != nodes or s_dec != nodes or res_dec != num_time or surplus_dec != 1:
        ...
        print(statement_stopping.format(iteration, r_dec, s_dec,
                                        res_dec, surplus_dec, terminator))
    ...
    ******************************
    ...
    with open(".\\runtime\\temp\\auction.json", "w") as outfile:
